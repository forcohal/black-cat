...
class entity {
    ...
    drawCat(){
        ...
        function lines(initX,initY,finX,finY){
            ...
            ctx.lineTo(xc - finX, yc - finY); // <- reduced from -800 so it's visible
    ...
    move() {
        if (keys["ArrowLeft"]){ this.x -= this.velx; this.moving = true;}
        else if (keys["ArrowRight"]){ this.x += this.velx; this.moving = true;}
    ...
    if (
        ...
        this.botLeft[1] <= platform.topLeft[1] + 10 // tolerance
    ) {
        this.inAir = false;
    ...
    // Bottom collision (hitting head)
        ...
        this.topLeft[1] <= platform.topLeft[1] + platform.height &&
        this.topLeft[1] >= platform.topLeft[1] + platform.height - 10
    ) {
        this.vely = 0;
        this.y = platform.topLeft[1] + platform.height;
        this.collided = true;
    ...
    // Left side collision (player moving right into platform)
        ...
        this.topRight[1] < platform.topLeft[1] + platform.height
    ) {
    ...
    // if(this.inTouch && this.won){
    //     resetLevel()
    // }

    else{this.won = false; this.restart = false; this.inTouch = false}
...
class Playground{
    ...
    update() {
        ...
        if(this.won){
            ...
            player1.isColliding(base)// then check collisions
            player1.move();
            player1.jump();
            player1.gravity();
            player1.draw();
            ...
            cat.canvas();               // keep player in canvas
            cat.coord();                // update player coords first
            //cat.isInAir(); 
            cat.isColliding(base);             // check ground only
            /* cat.isColliding(platform1); // then check collisions
            cat.isColliding(platform2); */ // then check collisions
            cat.gravity();
            //cat.draw();
            cat.drawCat()
